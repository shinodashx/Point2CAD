...
description: "仅通过 XYZ 数值和 Python 分析 NPY、PLY 或粘贴点云，以 CadQuery 特征序列重建并量化精度。用于明确要求不看点云图的纯数值点云到 CAD 重建；保留 CAD 产物检查，不使用网格转实体或整体包面。"
---

# 点云到 CadQuery 序列重建 · 纯数值 / Numerical-only

把点云当测量证据，而不是待转换的几何实体。**NPY、PLY、粘贴 XYZ 必须执行同一流程：读取 → 分析 → 特征与尺寸证据 → CAD 操作计划 → 序列建模 → 误差闭环。**

## 硬性规则

- **点云分析仅用 XYZ 数值和 Python。** 不生成或查看源点云投影、截面图、残差图或叠加图，不借用其他模式的图像结论。点云截面指坐标筛选后的数值计算。尺寸、轴线、平面、曲率及精度判断用可复现数值分析；局部拟合残差不等于最终CAD误差。保留与混合版相同的 CAD 产物渲染、拓扑及装配检查，但 CAD 图只能检查建模缺陷，不能代替源点云的数值证据。
...
1. **验收输入。** 文件直接读取；粘贴坐标原样保存。遇到截断、缺行、省略号先找完整附件，找不到就要求补全。记录原文件哈希、点数、索引、单位及公差；不静默缩放、转置、删点或去重。运行本技能读取命令，只查看坐标和统计；不能调用混合版会生成投影的读取器。
2. **重新理解。** 完整读取XYZ后，以坐标分布、平面分层、薄层截面和轴向—径向测量识别特征。建立主平面、轴、厚度、台阶、孔槽、凹槽、折弯和圆角；分别检验内外壁及端面，不从外形相似推定同轴、等厚或径向边界。PCA 仅给轴候选；记录局部→世界变换。用原点索引分区，不能丢掉稀疏小特征。
...
6. **闭环验证与迭代。** 全量点到导出CAD表面的距离，加分区／关键尺寸／孔槽／部件干涉检查；部件表面与融合表面分别报告。超差、系统性残差或无效实体必须按 [references/refinement.md](references/refinement.md) 定位→反证假设→修改操作→全量复验，不仅在报告里承认问题。保留逐轮序列、误差、超差点数与文件哈希；细化网格并按需复核STEP面。用 BRep 截面、邻面法向／切点、探针及误差表检查接合；另查看纯 CAD 导出物的整体、背面、截面和接合放大图，不叠加点云。
7. **交付。** 原输入来源与读取报告、证据表、计划、`sequence_cq.py`、STEP、需要的STL、逐轮比较、可复现验证与逐点误差、已查看的纯 CAD 渲染。核对报告与最终文件哈希一致；说明单位、目标、近似及未观测区域，并注明未使用点云可视化分析。达标或有记录的证据不足／停滞后才结束，未达标明确标为局部近似并指出需要补充什么数据，不承诺任意输入都能唯一恢复。只有要求运动时才增加运动脚本及状态检查。
...
`$SKILL_DIR` 为本技能目录。缺依赖时使用隔离环境；读取需 `numpy scipy plyfile`，验证需 `numpy vtk`（仅距离计算，不渲染），STEP 复核和建模需 `cadquery`。测试过 Python 3.11／CadQuery 2.8。输出目录须新建或为空，原输入不可覆盖。
