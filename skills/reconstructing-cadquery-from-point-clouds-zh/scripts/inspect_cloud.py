"""Read NPY/PLY/XYZ as measurement coordinates; never construct CAD geometry."""
...
def inspect(points, info, out, unit="unknown", unit_status="unknown"):
    ...
    sample = np.sort(np.random.default_rng(0).choice(len(points), min(len(points), 50000), replace=False))
    info.update(unit=unit, unit_status=unit_status, unit_conversion="none", unique_count=len(unique),
                ...
                exact_coordinate_modes=modes, projection_sample_count=len(sample), projection_seed=0)
    ...
    fig, panels = plt.subplots(1, 3, figsize=(13, 5), layout="constrained")
    for panel, (a, b) in zip(panels, [(0, 1), (0, 2), (1, 2)]):
        panel.scatter(points[sample, a], points[sample, b], s=2, alpha=.6)
        panel.set(xlabel="XYZ"[a], ylabel="XYZ"[b], title="XYZ"[a]+"XYZ"[b])
        panel.set_aspect("equal", adjustable="box")
        panel.grid(alpha=.2)
    fig.suptitle(f"Input measurements | {len(points)} points | units: {unit} ({unit_status})\n"
                 f"Projection: {len(sample)} points; coordinates unchanged, no CAD generated")
