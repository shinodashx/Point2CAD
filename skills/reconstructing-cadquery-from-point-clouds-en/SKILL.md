...
- **XYZ numerical evidence leads; images assist.** Use reproducible numerical analysis for dimensions, axes, planes, curvature, tangency, and accuracy. Views suggest candidate structures and reveal occlusion; do not estimate dimensions from pixels. Local fit residuals are not final CAD errors.
...
- Allow local plane/circle/cylinder/arc estimation for **measurement**, and optimization of a few meaningful dimensions within an evidenced feature structure. If evidence contradicts that structure, revise the feature hypothesis before tuning dimensions. Do not build the final model through per-point high-order splines or cloud wrapping.
...
2. **Understand afresh.** After reading all XYZ, identify features numerically through coordinate distributions, plane layers, thin slices, and axial–radial measurements, assisted by XY/XZ/YZ and multiple views. Identify planes, axes, thicknesses, steps, holes/slots, recesses, bends, and fillets. Test inner/outer walls and end faces separately; similar appearance does not establish coaxiality, uniform thickness, or radial boundaries. Treat PCA as axis candidates only; record local-to-world transforms. Partition using source indices without losing sparse small features.
3. **Record evidence.** Write `feature_evidence.csv`: feature/body, source-index file, support count and spatial coverage, measured/adopted parameters, fitted and adopted-parameter residuals, confidence, and assumptions. Save partition rules and measurement scripts; separate observed, inferred, and unknown geometry. Check nominal rounding, symmetry, and simplification numerically; minimal support alone does not imply high confidence.
...
6. **Validate and iterate.** Measure all points against exported CAD surfaces; add regional, dimensional, hole/slot, and component-interference checks. Report component and fused surfaces separately. For exceeded targets, systematic residuals, or invalid bodies, follow [references/refinement.md](references/refinement.md): localize → challenge the hypothesis → revise operations → revalidate all points, rather than merely documenting a problem. Preserve each sequence revision, errors, exceedance count, and file hashes; refine tessellation and cross-check STEP faces as needed. Render and inspect overall/back/section/junction views.
7. **Deliver.** Provide provenance/input report, evidence, plan, `sequence_cq.py`, STEP, requested STL, revision comparisons, reproducible validation/per-point errors, and inspected renders. Match reports to final file hashes; state units, targets, approximations, and unobserved regions. Stop only at acceptance or documented evidence limits/stagnation; label unmet regions as approximations and specify missing data, without promising unique recovery for arbitrary inputs. Add motion scripts/state checks only when motion is requested.
